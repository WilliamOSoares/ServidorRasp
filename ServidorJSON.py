...
def refinaEnviaDado(cicloLeitura):
	global dadosDaLeitura, conectado, ini, fim, trava, cont
	trava.acquire()	
	tempo = fim-ini
	if(len(dadosDaLeitura)>0 and tempo>cont):
		cont=cont+5
		preparajson = '{'
...
def qualificatorio(con, produtor, consumidor):
	global dadosDaLeitura, tempoQuali, cicloLeitura, trava, online, conectado, reader, ini, fim
	reader = iniciaLeitor()
	reader.start_reading(lambda tag: dadosLeitura(tag.epc, tag.rssi, datetime.fromtimestamp(tag.timestamp)))
	cicloLeitura = 0
	tempo = tempoQuali
